...
@router.post("/", response_model=SuccessResponse[ContentInfo])
...
@router.get("/{content_id}", response_model=SuccessResponse[ContentInfo])
...
@router.put("/{content_id}", response_model=SuccessResponse[ContentInfo])
...
@router.get("/", response_model=PaginationResponse[ContentInfo])
async def get_content_list(
    ...
    # 时间筛选参数
    publish_date_start: Optional[str] = Query(None, description="发布开始日期 (YYYY-MM-DD)"),
    publish_date_end: Optional[str] = Query(None, description="发布结束日期 (YYYY-MM-DD)"),
    create_date_start: Optional[str] = Query(None, description="创建开始日期 (YYYY-MM-DD)"),
    create_date_end: Optional[str] = Query(None, description="创建结束日期 (YYYY-MM-DD)"),
    
    # 付费筛选参数
    is_free: Optional[bool] = Query(None, description="是否免费内容"),
    is_vip_free: Optional[bool] = Query(None, description="是否会员免费"),
...
@router.post("/{content_id}/chapters", response_model=SuccessResponse[ChapterInfo])
...
@router.post("/{content_id}/purchase", response_model=SuccessResponse[UserContentPurchaseInfo])
...
@router.get("/my/contents", response_model=PaginationResponse[ContentInfo])
...
@router.get("/hot", response_model=PaginationResponse[ContentInfo])
...
@router.get("/search", response_model=PaginationResponse[ContentInfo])
async def search_contents(
    q: str = Query(..., min_length=1, description="搜索关键词"),
    content_type: Optional[str] = Query(None, description="内容类型"),
    category_id: Optional[int] = Query(None, description="分类ID"),
    sort_by: str = Query("view_count", description="排序字段：create_time, update_time, publish_time, view_count, like_count, favorite_count, comment_count, score"),
    sort_order: str = Query("desc", description="排序方向：asc, desc"),
    page: int = Query(1, ge=1, description="页码"),
    size: int = Query(20, ge=1, le=100, description="每页数量"),
    db: AsyncSession = Depends(get_async_db)
