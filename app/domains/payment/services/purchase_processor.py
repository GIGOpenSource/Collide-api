...
class PaymentPurchaseProcessor:
    """支付成功后的商品购买处理器"""
    
    def __init__(self, db: AsyncSession):
        self.db = db
    
    async def process_payment_success(self, order_no: str) -> dict:
        """
        处理支付成功后的商品购买业务逻辑
        ...
        try:
            ...
            if order.pay_status != "paid":
                logger.warning(f"订单未支付，无法处理: {order_no}")
                return {"error": "订单未支付，无法处理"}
            
            # 获取商品信息
            goods = await self._get_goods_by_id(order.goods_id)
            if not goods:
                logger.warning(f"商品不存在: {order.goods_id}")
                return {"error": "商品不存在"}
            ...
            if order.goods_type == "subscription":
                ...
                # 内容商品通过金币购买，不在此处处理
                # 内容购买逻辑在用户钱包模块中处理
                result["processed_items"].append("content_skipped")
            ...
            logger.info(f"支付成功处理完成: {result}")
            return result
            
        except Exception as e:
            logger.error(f"处理支付成功逻辑失败: {str(e)}", exc_info=True)
    ...
    async def _process_subscription_purchase(self, order: Order, goods: Goods):
        ...
        logger.info(f"用户 {order.user_id} VIP订阅处理成功，过期时间: {new_expire_time}")
    ...
    async def _process_coin_purchase(self, order: Order, goods: Goods):
        ...
        await self.db.commit()
        logger.info(f"用户 {order.user_id} 金币充值成功，获得金币: {earned_coins}")
    
    async def _process_content_purchase(self, order: Order, goods: Goods):
        """处理内容商品购买（已移至用户钱包模块）"""
        logger.info(f"内容购买逻辑已移至用户钱包模块，跳过处理: 用户 {order.user_id}, 内容 {goods.content_id}")
        pass
    ...
    async def _process_goods_purchase(self, order: Order, goods: Goods):
        ...
        # 例如：发送通知、记录购买历史等
        logger.info(f"用户 {order.user_id} 普通商品购买成功: {goods.name}")
